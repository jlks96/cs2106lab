...
TBuffer buffer;//create a TBuffer 

int main(int ac, char **av)
{
	initBuffer(&buffer);//initialise the buffer
...
void writeLog(const char *format, ...)
{
	...
	sprintf(myBuffer, "%s: %s\n", getCurrentTime(), myBuffer);

	for (int i = 0; i < LOG_BUFFER_LEN; i++) {
		int remainingBytes = LOG_BUFFER_LEN - i;
		unsigned int bytesToCopy = (remainingBytes < ENTRY_SIZE ? remainingBytes : ENTRY_SIZE);
		char data[ENTRY_SIZE];
		memcpy(data, &myBuffer + i, bytesToCopy);
