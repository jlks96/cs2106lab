...
int RMSScheduler()
{
	...
	//if no process is running, dequeue from the ready queue
	/*
	if (currProcessNode == NULL) {
		currProcessNode = prioRemoveNode(&readyQueue, currProcessNode);
	...
	*/
	...
	&processes[firstInReadyNode->procNum];
	...
	if (firstInReadyNode->prio < currProcessNode->prio) {
		...
		firstInReadyProcess->timeLeft = firstInReadyProcess->c;
		...
		//if current process has not finished running for the current run
